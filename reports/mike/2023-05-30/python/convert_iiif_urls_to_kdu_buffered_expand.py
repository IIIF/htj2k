...
#iiif_urls_filename_default = './resources/iiif_urls/uv_urls.txt'
#iiif_urls_filename_default = './resources/iiif_urls/mirador_urls.txt'
#iiif_urls_filename_default = './resources/iiif_urls/iiif_urls_unique.txt'
#iiif_urls_filename_default = './resources/iiif_urls/full.txt'
#iiif_urls_filename_default = './resources/iiif_urls/custom_region-100-100-2000-2000.txt'
#iiif_urls_filename_default = './resources/iiif_urls/custom_region-100-100-200-200.txt'
#iiif_urls_filename_default = './resources/iiif_urls/3000.txt'
#iiif_urls_filename_default = './resources/iiif_urls/1024.txt'
#iiif_urls_filename_default = './resources/iiif_urls/1000.txt'
#iiif_urls_filename_default = './resources/iiif_urls/500.txt'
#iiif_urls_filename_default = './resources/iiif_urls/50.txt'
...
number_of_decoding_cycle_iterations_default = 5
...
#output_script_file.write("set -e\n")
...
for line in lines:
    ...
    if ("full" == words[2]):
      ...
      rendered_w = int(iiif_json_dictionary['width'])
      rendered_h = int(iiif_json_dictionary['height'])
      is_rendered_w_provided = True
      ...
      if( rendered_wh_tokens[0] == ""):
        # rendered height wasn't provided
        is_rendered_w_provided = False
      else:
    ...
    if( is_rendered_w_provided == True):
      scaling_factor_not_power_of_two = full_resolution_w / rendered_w
    else:
      scaling_factor_not_power_of_two = full_resolution_h / rendered_h
    scaling_factor_log2 = int( round( math.log2(scaling_factor_not_power_of_two) ) )

    # limit digital bodelian decoding to reduce 6 since Clevels=6 is used as a digital bodelian encoding parameter
    if( scaling_factor_log2 > 6 and "bodelian" in compressed_directory ):
      scaling_factor_log2 = 6
      if( is_rendered_w_provided == True):
        rendered_w = full_resolution_w / math.pow(2, scaling_factor_log2)
      if( is_rendered_h_provided == True):
        rendered_h = full_resolution_h / math.pow(2, scaling_factor_log2)
    ...
    if( is_rendered_w_provided == False ) :
      rendered_w = int( round( float(full_resolution_w) / float(scaling_factor) ) )
    # compute rendered_h if it wasn't provided
